...
def get_ollama_response(question, prompt):
    full_prompt = prompt[0].format(question=question)
    data = {
        "model": "llama3.1:8b",
        "prompt": full_prompt,
        "stream": False
...
def determine_chart_type(df):
    # Check for bar chart (2 columns, second column is numeric, more than 1 row)
    if len(df.columns) == 2:
        if df.dtypes[1] in ['int64', 'float64'] and len(df) > 1:
            return 'bar'
        # Check for pie chart (2 columns, second column is numeric, 10 or fewer rows)
        elif df.dtypes[1] in ['int64', 'float64'] and len(df) <= 10:
            return 'pie'
    
    # Check for line chart (3 or more columns, second column is numeric)
    elif len(df.columns) >= 3 and df.dtypes[1] in ['int64', 'float64']:
        if df.columns[0].lower() in ['month', 'monthid', 'year', 'date', 'time', 'day']:
          return 'line'
    
    # Check for scatter plot (2 numeric columns, more than 1 row)
    if len(df.columns) == 2 and df.dtypes[0] in ['int64', 'float64'] and df.dtypes[1] in ['int64', 'float64'] and len(df) > 1:
        return 'scatter'
    
    # Check for histogram (1 numeric column)
    if len(df.columns) == 1 and df.dtypes[0] in ['int64', 'float64']:
        return 'histogram'
    
    # Check for heatmap (correlation matrix or 2D numeric data)
    if len(df.columns) > 1 and all(df.dtypes[i] in ['int64', 'float64'] for i in range(len(df.columns))):
        return 'heatmap'
    
    # Check for area chart (2 columns, second column is numeric)
    if len(df.columns) == 2 and df.dtypes[1] in ['int64', 'float64']:
        return 'area'
    
    # Check for bubble chart (3 columns, third column is numeric)
    if len(df.columns) == 3 and df.dtypes[2] in ['int64', 'float64']:
        return 'bubble'
    
    # Check for dot plot (2 columns, second column is numeric)
    if len(df.columns) == 2 and df.dtypes[1] in ['int64', 'float64']:
        return 'dot'
    
    # Check for treemap (2 columns, second column is numeric)
    if len(df.columns) == 2 and df.dtypes[1] in ['int64', 'float64']:
        return 'treemap'
    
    # Check for gauge chart (2 columns, second column is numeric, small number of rows)
    if len(df.columns) == 2 and df.dtypes[1] in ['int64', 'float64'] and len(df) <= 5:
        return 'gauge'
    
    # Check for radar chart (2 columns, second column is numeric)
    if len(df.columns) == 2 and df.dtypes[1] in ['int64', 'float64']:
        return 'radar'
    
    # If no suitable chart type is found
    return None
...
def generate_chart(df, chart_type):
    ...
    if df.empty:
        st.write("The DataFrame is empty. Cannot generate a chart.")
        return
    ...
    try:
        if chart_type == 'bar':
            ...
            if df.select_dtypes(include=['int64', 'float64']).shape[1] > 1:
                ...
                st.write("Heatmap requires at least two numeric columns.")
                return
        
        elif chart_type == 'area':
            fig = px.area(df, x=df.columns[0], y=df.columns[1],
                          title=f"{df.columns[1]} Over {df.columns[0]} (Area Chart)",
                          template="plotly_white")
        
        elif chart_type == 'bubble':
            fig = px.scatter(df, x=df.columns[0], y=df.columns[1], size=df.columns[2],
                             title=f"Bubble Chart: {df.columns[0]} vs. {df.columns[1]}",
                             template="plotly_white", color=df.columns[0])
        
        elif chart_type == 'dot':
            fig = px.scatter(df, x=df.columns[0], y=df.columns[1],
                             title=f"Dot Plot: {df.columns[0]} vs. {df.columns[1]}",
                             template="plotly_white", color=df.columns[0])
        
        elif chart_type == 'treemap':
            fig = px.treemap(df, path=[df.columns[0]], values=df.columns[1],
                             title=f"Treemap of {df.columns[0]}",
                             template="plotly_white")
        
        elif chart_type == 'gauge':
            fig = go.Figure(go.Indicator(
                mode="gauge+number",
                value=df.iloc[:, 1].mean(),  # Using mean value as an example
                title={"text": f"Gauge Chart: {df.columns[0]}"},
                gauge={"axis": {"range": [0, df.iloc[:, 1].max()]}}
            ))
        
        elif chart_type == 'radar':
            fig = px.line_polar(df, r=df.columns[1], theta=df.columns[0],
                                title=f"Radar Chart: {df.columns[0]}",
                                template="plotly_white", line_close=True)
